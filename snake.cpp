...
void snake::move()
{
  ...
  while (1)
  {
    ...
    if (kbhit())
    {
      ...
      int new_dir = getch();
      switch (new_dir) {
          case 72: if (direction != 80) direction = new_dir; break;
          case 80: if (direction != 72) direction = new_dir; break;
          case 75: if (direction != 77) direction = new_dir; break;
          case 77: if (direction != 75) direction = new_dir; break;
          default: break;
      }
    }
    ...
    // }
    // 边界/墙：棋盘外（0或21）直接判失败；
    if (x == 0 || x == H - 1 || y == 0 || y == W - 1) // 撞墙 over
    {
      system("cls");
      cout << "\tGame over!" << endl;
